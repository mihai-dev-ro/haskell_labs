...
instance Functor (Reader r) where
    fmap f (Reader fn) = Reader (\r -> f $ fn r)

instance Applicative (Reader r) where
    pure a = Reader (\r -> a)
    Reader fn <*> Reader fn' = Reader $ \r -> fn r $ fn' r

instance Monad (Reader r) where
    return = pure

    -- (>>=) :: Reader r a -> (a -> Reader r b) -> Reader r b
    Reader fn >>= f = Reader $ \r -> runReader (f $ fn r) r
...
local f reader = Reader $ \r -> 
    let r' = f r
    in runReader reader r' 
